...
public class TopMainBarFinder implements MainBarFinder {

    SafeLapPointFinder safeLapPointFinder;
    RealBarArranger realBarArranger;

    public TopMainBarFinder(SafeLapPointFinder safeLapPointFinder, RealBarArranger realBarArranger) {
    ...
    public List<SolutionMainBar> find(BeamInputData inputData) {
        ...
        List<SafeLapPoint> safePoints = safeLapPointFinder.findSafeLapPointsForTop(inputData);
        ...
        findApplicableTopMainBars(inputData, safePoints, resultList, currentBar, 0, beginRange);
        ...
        System.out.println("TOTAL OF ALL POSSIBLE SOLUTION IS:" + solutionMainBarList.size());
        ...
        DesignBeamHelpers.generateSubsets(solutionMainBarList.size(), 9, solutionMainBarList);

        return solutionMainBarList;
    }

    private void findApplicableTopMainBars(BeamInputData inputData, List<SafeLapPoint> safePoints, List<TopMainBar> resultList,
        ...
        findApplicableTopMainBars(inputData, safePoints, resultList, currentBar, currentSafePointIdx + 1, newBeginRange);

        currentBar.getBars().remove(currentBar.getBars().size() - 1);
        //dont have overlap on current safePointIdx
        findApplicableTopMainBars(inputData, safePoints, resultList, currentBar, currentSafePointIdx + 1, beginRange);
    }
}
